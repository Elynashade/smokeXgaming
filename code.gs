...
function handleSignUp(sheet, rows, data) {
  ...
  if (emailExists) {
    return createResponse({ result: "error", message: "This email is already registered." });
  }
...
function sendOTPEmail(email, code) {
  const htmlBody = `
    <div style="font-family: Arial, sans-serif; padding: 20px; color: #333;">
      <h2 style="color: #000;">SmokeX Gaming Verification</h2>
      <p>Hello,</p>
  ...
  GmailApp.sendEmail(email, code + " is your SmokeX Gaming verification code", "", {
    name: "SmokeX Gaming Arena",
    to: email,
    htmlBody: htmlBody
  });
}
