document.addEventListener('DOMContentLoaded', () => {
    ...
    hamburger.addEventListener('click', () => toggleHamburgerMode(false));
    ...
    partnerCircles.forEach(circle => {
        circle.addEventListener('mouseenter', () => {
            ...
            gsap.to(partnerDesc, {
                ...
                onComplete: () => {
                    ...
                    partnerDesc.style.borderLeft = "1px solid var(--gold)";
        ...
        circle.addEventListener('mouseleave', () => {
            gsap.to(partnerDesc, { opacity: 0.6, y: 0, duration: 0.4, onStart: () => {
                partnerDesc.innerText = defaultPartnerText;
    ...
    const regTriggers = document.querySelectorAll('.reg-trigger');
    const closeAuth = document.querySelector('.close-auth');
    const authTabs = document.querySelectorAll('.auth-tab');
    const authForms = document.querySelectorAll('.auth-form');

    const openAuth = (targetForm = 'login-form') => {
        gsap.to(authOverlay, { autoAlpha: 1, duration: 0.5, ease: "power4.out" });
    ...
    loginTriggers.forEach(btn => btn.addEventListener('click', () => openAuth('login-form')));
    regTriggers.forEach(btn => btn.addEventListener('click', () => openAuth('register-form')));
    ...
    const initGallery = () => {
        ...
        const fWidth = isMobile ? window.innerWidth * 0.85 : 350;
        const fHeight = isMobile ? 380 : 500;
        const tWidth = isMobile ? 60 : 150;
        const tHeight = isMobile ? 90 : 220;
        const gap = 30, numberSize = 50;
        ...
        const initPositions = () => {
            ...
            const featuredX = isMobile ? (w/2 - fWidth/2) : w * 0.62; 
            const featuredY = isMobile ? 250 : (h / 2) - (fHeight / 2);
            
            // Starting point for the queue (Far Right Bottom)
            const queueX = isMobile ? (w/2 - ((gameData.length-1)*tWidth)/2) : w * 0.82;
            ...
            gsap.set("#details-odd", { opacity: 0, visibility: 'hidden', x: -30 });
            gsap.set("#details-even", { opacity: 1, visibility: 'visible', x: 0 });
        ...
        const step = (direction = 'next') => {
            ...
            gsap.set(activeDetails, { zIndex: 25, opacity: 0, visibility: 'visible', x: -30 });
            gsap.to(activeDetails, { opacity: 1, x: 0, duration: 0.6, ease: "power4.out" });
            gsap.to(inactiveDetails, { opacity: 0, duration: 0.4, onComplete: () => gsap.set(inactiveDetails, { visibility: 'hidden' }) });

            const h = window.innerHeight, w = window.innerWidth;
            const featuredX = w * 0.62, featuredY = (h / 2) - (fHeight / 2);
