document.addEventListener('DOMContentLoaded', () => {
    ...
    const staggerSubMenu = (item) => {
        ...
        if (spans.length > 0) {
            gsap.fromTo(spans, 
                { opacity: 0, y: 10 }, 
                { opacity: 1, y: 0, stagger: 0.1, duration: 0.4, ease: "back.out(1.7)", overwrite: true }
            );
        }
    };
    ...
    const initGallery = () => {
        ...
        const fWidth = isMobile ? window.innerWidth * 0.7 : 350;
        const fHeight = isMobile ? fWidth * 1.4 : 500;
